...
export default function ChatPage() {
  ...
  const handleSelectConversation = (selectedUser: User, conversationId: string) => {
    ...
    setActiveTab("chats"); // Switch back to chats when a conversation is selected
  };
  ...
  return (
    <div className="h-screen w-full flex bg-gradient-to-br from-[var(--color-bg)] to-indigo-500/5 overflow-hidden">
      {/* Navigation Rail */}
      <div className="w-20 bg-[var(--color-glass-bg)] backdrop-blur-3xl border-r border-[var(--color-glass-border)] flex flex-col items-center py-8 shadow-[2px_0_12px_-4px_rgba(0,0,0,0.1)] z-40 transition-colors duration-300 m-3 rounded-[2.5rem]">
        ...
        <div className="flex flex-col space-y-4 flex-1">
          {navItems.map(({ tab, icon: Icon, label }) => (
            <button
              ...
              className={`relative w-12 h-12 flex items-center justify-center rounded-[1.25rem] transition-all hover:scale-110 active:scale-95 ${
                activeTab === tab
      ...
      <div className="flex-none m-3 ml-0 rounded-[2.5rem] overflow-hidden border border-[var(--color-glass-border)] shadow-xl transition-all duration-300">
        {activeTab === "chats" && (
          <Sidebar 
      ...
      {/* Chat Window */}
      <div className="flex-1 m-3 ml-0 rounded-[2.5rem] overflow-hidden border border-[var(--color-glass-border)] shadow-xl transition-all duration-300">
