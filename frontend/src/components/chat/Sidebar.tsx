    ...
    <div className="w-full md:w-80 h-full bg-[var(--color-sidebar)]/80 backdrop-blur-3xl border-r border-[var(--color-glass-border)] flex flex-col z-30 shadow-[4px_0_24px_-10px_rgba(0,0,0,0.1)] relative">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-2 space-y-2 pointer-events-none">
        {toasts.map(toast => (
          ...
          >
            <div className="w-9 h-9 rounded-[12px] border-2 border-dashed flex items-center justify-center font-black text-sm shrink-0" style={{ backgroundColor: 'var(--color-bg)', borderColor: getAvatarGradient(toast.sender), color: getAvatarGradient(toast.sender) }}>
        ...
        {searchQuery ? (
          <div>
            ...
            {searchResults.map(u => (
              ...
              >
                <div className="w-12 h-12 rounded-[16px] border-2 border-dashed flex items-center justify-center font-black shrink-0 transition-transform text-lg" style={{ backgroundColor: 'var(--color-bg)', borderColor: getAvatarGradient(u.name), color: getAvatarGradient(u.name) }}>
            ...
            return (
              ...
              >
                <div className="relative shrink-0">
                  <div className={`w-12 h-12 rounded-[16px] border-2 border-dashed flex items-center justify-center font-black text-xl shadow-sm transition-transform group-hover:scale-105 ${isDoodleMode ? 'group-hover:animate-wobbly' : ''}`} style={{ backgroundColor: 'var(--color-bg)', borderColor: getAvatarGradient(otherUser.name), color: getAvatarGradient(otherUser.name) }}>
